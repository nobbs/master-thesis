...
Hierbei werden Raum- und Zeitkoordinaten oftmals verschieden behandelt, das heißt die Variationsformulierung wird nur bezüglich der Raumkoordinaten hergeleitet \todo{Formulierung und Quelle?} und die Zeitkoordinate wird separat behandelt.
...
Multiplizieren wir nun die partielle Differentialgleichung \eqref{eq:allgemeine_parabolische_pde} mit $v = (v_{1}, v_{2}) \in \mathcal Y$ und integrieren über den Zeitintervall $I$, \todo{Herleitung aus-x-en?} dann erhalten wir daraus das Variationsproblem:
...
Die Konstante $M_{e}$ ist definiert \todo{genauer?} durch
\begin{equation}
...
\begin{Annahme}[{{\cite[Assumption 2]{Kunoth:2013ef}}}]
    ...
    \begin{thmenumerate}
        \item Der \emph{mean field}-Operator\todo{Übersetzung} ~$A_{0} \in \mathcal L(X, Y')$ sei stetig invertierbar, das heißt es existiert ein $\gamma_{0} > 0$ mit
        \begin{equation}
        ...
        \end{equation}
        \item Die \emph{fluctuation}-Operatoren\todo{Übersetzung} ~$\{ A_{j} \}_{j \geq 1}$ seien \emph{klein} relativ zu $A_{0}$ im folgenden Sinne: es existiert eine Konstante $0 < \kappa < 1$ so dass
        \begin{equation}
...
\begin{Satz}[{{\cite[Theorem 21]{Kunoth:2013ef}}}]
    ...
    \end{equation}
    Die parametrische Familie von Operatoren $\{ B(\sigma) : \sigma \in \mathcal S \}$ erfüllt \thref{thm:kunoth:assumption1} mit dem selben Regularitätsparameter $\mathfrak p$ und die parametrische Familie von Lösungen $u(\sigma)$ erfüllt die \emph{a priori}-Abschätzung
    \begin{equation}
