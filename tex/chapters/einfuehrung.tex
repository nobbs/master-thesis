...
Weiterhin sei $a \colon V \times V \to \mathbb{R}$ eine Bilinearform und $A$ der zugehörige, durch $\skprod{A \eta}{\zeta}_{H} \deq a(\eta, \zeta)$, $\eta, \zeta \in V$, induzierte lineare Operator.
...
In den nachfolgenden Abschnitten werden wir von der Bilinearform $a$ die folgenden zwei Eigenschaften fordern.

\begin{Annahme}
\label{annahme:eigenschaften_bf_a}
    Die Bilinearform $a \colon V \times V \to \mathbb{R}$ erfülle:
    \begin{thmenumerate}
...
\end{equation}
definiert sind.
Nach \thref{satz:gl:einbettung_bochner_stetig} gilt $\mathcal X \hookrightarrow \mathcal C([0, T]; H)$ und $M$, wobei aus dem Beweis in \cite{Dautray:1992by} zudem folgt, dass $M_{e}$ gleichmäßig beschränkt in der Wahl von $V \hookrightarrow H$ ist und nur im Fall $T \to 0$ von $T$ abhängt.
