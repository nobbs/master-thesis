...
\setchapterpreamble[ul][0.6\textwidth]{%
    \dictum[Andy Weir, \textit{The Martian}]{\enquote{I guess you could call it a "failure", but I prefer the term "learning
experience".}}
...
In diesem Kapitel konkretisieren wir die parabolische partielle Differentialgleichung aus \autoref{cha:einleitung} und reinterpretieren diese als eine parametrische Differentialgleichung.
...
% Hierbei orientieren wir uns an den Arbeiten von !TODO!.

\section{Problemstellung} % (fold)
\label{sec:problemstellung}

\subsection{Konkretisierung} % (fold)
\label{sub:konkretisierung}

% subsection konkretisierung (end)

Wir greifen nun die parabolische partielle Differentialgleichung aus \autoref{cha:einleitung} auf und leiten daraus die für diese Arbeit relevante Problemstellung her.

Sei also $0 < T < \infty$ eine Konstante und $I = [0, T]$ ein reelles Intervall.
Weiter sei $\Omega \subset \mathbb{R}^{n}$ eine beschränkte abgeschlossene Menge mit Lipschitz-Rand !TODO!.
Wir betrachten nun die partielle Differentialgleichung
\begin{equation}
    u_{t}(t, x) = c \Delta_{x} u(t, x) - \omega(t, x) u(t, x) - \mu u(t, x) \quad \text{auf}~I \times \Omega,
\end{equation}
...
An dieser Stelle beschränken wir uns auf den Fall homogener Randbedingungen in $\Omega$, das heißt es gilt $\restr{u(t, \blank)}{\partial \Omega} = 0$ für alle $t \in I$.

\todo[inline]{Das Mu erklären!}

Unter diesen Gegebenheiten können wir die obige partielle Differentialgleichung konkretisieren; es bietet sich an, dazu die Räume $V = H^{1}_{0}(\Omega)$ und $H = L_{2}(\Omega)$ zu verwenden.
Dabei handelt es sich um separable Hilberträume und es existiert eine dichte stetige Einbettung $H^{1}_{0}(\Omega) \denseinclusion L_{2}(\Omega)$.
Nach \thref{definition:gl:gelfand_tripel} liefert dies das Gelfand-Tripel
\begin{equation}
    H^{1}_{0}(\Omega) \denseinclusion L_{2}(\Omega) \denseinclusion (H^{1}_{0}(\Omega))' = H^{-1}(\Omega).
\end{equation}

Mit diesen Vorbemerkungen erhalten wir nun unter Hinzunahme eines Quellterms und einer Anfangsbedingung das folgende Problem.

\begin{Definition}[Starke Formulierung]
\label{definition:pp:starke_formulierung}
    Seien eine \emph{Anfangsbedingung} $u_{0} \in L_{2}(\Omega)$, ein \emph{Quellterm} $g \in L_{2}(I) \times H^{-1}(\Omega)$ und $\omega$ wie in \eqref{eq:pp:def_omega} gegeben.
...
Wir wollen nun eine Raum-Zeit-Variationsformulierung, auch schwache Formulierung genannt, von \eqref{eq:pp:starke_formulierung} herleiten.
Dazu ist es nützlich, den Operator $A(t)$ in Form einer Bilinearform zu schreiben.
Betrachte dazu die folgende duale Paarung auf $H^{-1}(\Omega) \times H^{1}_{0}(\Omega)$
\begin{equation}
    (A(t)\eta)\zeta = \dual{A(t)\eta}{\zeta},
\end{equation}
...
so dass
\begin{equation}
    (A(t)\eta)\zeta = \dual{A(t)\eta}{\zeta} = a(\eta, \zeta; t) \quad \fa \eta, \zeta \in H^{1}(\Omega)
\end{equation}
gilt.

\todo[inline]{Was geht hier ab?!}
Für den gegebenen Fall lässt sich diese Bilinearform explizit angeben, denn es gilt !TODO!
\begin{equation}
    \begin{aligned}
        a(\eta, \zeta; t)
        &=    \dual{A(t)\eta}{\zeta}
    ...
    \end{aligned}
\end{equation}
wobei bei der letzten Gleichheit der Gaußsche Intregalsatz !TODO! verwendet wurde.


Unter diesen Bedingungen erhalten wir für die obige Bilinearform für ein festes $t \in I$ die folgende Aussage.
%
\begin{Satz}
...
\begin{Korollar}
    Ist $\mu \geq \norm{\omega}_{L_{\infty}(\Omega)}$, dann ist die Bilinearform $a$ aus \thref{satz:pp:a_bf_bounded_garding} koerziv.
\end{Korollar}
...
Diese Variationsformulierung wird uns in den nachfolgenden Kapiteln als Ausgangspunkt dienen.

\subsection{Existenz und Eindeutigkeit von Lösungen} % (fold)
\label{sub:existenz_und_eindeutigkeit_von_l_sungen}

In diesem Abschnitt wollen wir für die gegebene Raum-Zeit-Variationsformulierung die Existenz und Eindeutigkeit von Lösungen nachweisen.
Dabei werden wir auf die Aussagen aus \thref{sec:lineare_evolutionsgleichungen}, vor allem das Banach-Ne{\v c}as-Babu{\v s}ka-Theorem, zurückgreifen.
