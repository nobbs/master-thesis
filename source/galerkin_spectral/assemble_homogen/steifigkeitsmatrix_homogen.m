...
function [M] = assemble_first_part(data, opt)
% Erste Teilmatrix: \int_Omega \int_I u * v dt dx
    ...
    for kdx = 1:opt.num.Xk
        for mdx = 1:kdx
            if kdx > mdx && mod(kdx + mdx, 2) == 1
                ...
                Idy(ctr) = (jdx - 1) * opt.num.Ym + mdx;
                Val(ctr) = (data.xspan(2) / 2) * 2 / (data.tspan(2) - data.tspan(1));
                ctr = ctr + 1;
            end
        end
    end
