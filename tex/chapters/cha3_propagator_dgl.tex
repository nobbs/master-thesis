...
Dies erreichen wir durch folgende Einschränkung der verwendeten Felder $w \in L_{\infty}(\Omega)$ auf Abbildungen, die wir als Reihenentwicklung von hier noch nicht näher spezifizierten Abbildungen $\varphi_{i}$ darstellen können.
...
\label{definition:feld_entwickelbar}
    Sei $\Set{\varphi_{i}}_{i \in \mathbb{N}} \subset L_{\infty}(\Omega)$ ein System von Funktionen und sei weiter ein Parameterraum $\mathcal P \subset \mathbb{R}^{\mathbb{N}}$ gegeben.
    ...
    \begin{equation}
        w(\blank; \bm\sigma) = \sum_{i = 1}^{\infty} \sigma_{i} \varphi_{i}
    \end{equation}
    übereinstimmt.
\end{Definition}
...
Dies ist aber auch nicht nötig, da, wie man in der Einführung bereits gesehen hat, die während des Iterationsverfahrens tatsächlich auftretenden Felder gewisse Eigenschaften, beispielsweise Symmetrien, aufweisen, und wir diese in die Wahl des Systems $\Set{\varphi_{i}}_{i \in \mathbb{N}}$ einfließen lassen können.
