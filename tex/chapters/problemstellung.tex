...
Bezeichne mit $a(\blank, \blank)$ die zu $A$ zugehörige Bilinearform, es gilt also
\begin{equation}
    a(\blank, \blank) \colon H^{1}_{0}(\Omega) \times H^{1}_{0}(\Omega) \to \mathbb{R}, \quad a(\eta, \zeta) = \skprod{A \eta}{\zeta}_{H^{-1}(\Omega) \times H^{1}_{0}(\Omega)}.
\end{equation}
Unter Verwendung der Greenschen Formeln erhalten wir
\begin{equation}
    \begin{aligned}
        a(\eta, \zeta)
        &= \skprod{A \eta}{\zeta}_{H^{-1}(\Omega) \times H^{1}_{0}(\Omega)}
        \\&= \skprod{- c \Delta \eta + \omega \eta}{\zeta}_{H^{-1}(\Omega) \times H^{1}_{0}(\Omega)}
        \\&= - c \skprod{\Delta \eta}{\zeta}_{H^{-1}(\Omega) \times H^{1}_{0}(\Omega)} + \skprod{\omega \eta}{\zeta}_{H^{-1}(\Omega) \times H^{1}_{0}(\Omega)}
...
\begin{Problem}
    ...
    \begin{equation}
        \label{eq:buv}
        b(u, v)
            = \int_{I} \skprod{u_{t}(t)}{v_{1}(t)}_{H^{-1}(\Omega) \times H^{1}_{0}(\Omega)} + a(u(t), v_{1}(t)) \diff t + \skprod{u(0)}{v_{2}}_{L_{2}(\Omega)}
    \end{equation}
    gegebene Bilinearform und $f(\blank) \colon \mathcal Y \to \mathbb{R}$ definiert ist durch
    \begin{equation}
        \label{eq:var_all_f_wiederholung}
        f(v) = \int_{I} \skprod{g(t)}{v_{1}(t)}_{H^{-1}(\Omega) \times H^{1}_{0}(\Omega)} \diff t + \skprod{u_{0}}{v_{2}}_{L_{2}(\Omega)}.
    \end{equation}
\end{Problem}
...
\begin{equation}
    ...
    A(\sigma) = \overline A + \sum_{j \geq 1} \sigma_{j} A_{j}
\end{equation}
auffassen, beziehungsweise als Bilinearformen
\begin{equation}
     \label{eq:aff_zerelgung_A_bf}
     a(\eta, \zeta; \sigma) = \overline a(\eta, \zeta) + \sum_{j \geq 1} \sigma_{j} a_{j}(\eta, \zeta), \quad \eta, \zeta \in V.
 \end{equation}
...
\begin{equation}
    ...
    \overline A = - c \Delta, \qquad
    A_{j} = \varphi_{j}, \quad j \geq 1.
\end{equation}
Die zugehörigen Bilinearformen lassen sich ebenfalls direkt angeben, denn es gilt
\begin{equation}
    \overline a(\eta, \zeta) = \skprod{\grad \eta}{\grad \zeta}_{L_{2}(\Omega)}, \qquad a_{j}(\eta, \zeta) = \skprod{\varphi_{j} \eta}{\zeta}_{L_{2}(\Omega)}, \quad j \geq 1.
\end{equation}
...
\begin{Problem}
    ...
    \begin{equation}
        \label{eq:buv_2}
        b(u, v; \sigma)
            = \int_{I} \skprod{u_{t}(t)}{v_{1}(t)}_{H^{-1}(\Omega) \times H^{1}_{0}(\Omega)} + a(u(t), v_{1}(t); \sigma) \diff t + \skprod{u(0)}{v_{2}}_{L_{2}(\Omega)}
    \end{equation}
    gegebene Bilinearform und $f(\blank) \colon \mathcal Y \to \mathbb{R}$ definiert ist durch
    \begin{equation}
...
\label{satz:regularitaet_nachrechnen}
    ...
    \begin{Beweis}
        Wir weisen zunächst die inf-sup-Bedingungen \eqref{eq:kunoth:ass2_gamma_0} für $\overline a(\blank, \blank)$ nach und bestimmen die Konstante $\gamma_{0}$.
        Da $\overline a(\blank, \blank)$ symmetrisch ist, genügt es die inf-sup-Bedingung \eqref{eq:kunoth:ass2_gamma_0_a} nachzuweisen. Die zweite inf-sup-Bedingung \eqref{eq:kunoth:ass2_gamma_0_b} folgt dann analog mit dem selben $\gamma_{0}$.
        ...
        Es ergibt sich
        \begin{align}
            \overline a(u, v_{u}) = \overline a(u, u) = c \skprod{\grad u}{\grad u}_{L_{2}(\Omega)} = c \norm{\grad u}_{L_{2}(\Omega)}^{2} \geq c \gamma_{\Omega}^{2} \norm{u}_{H^{1}(\Omega)}^{2},
        \end{align}
        ...
        \begin{align}
            ...
            \leq K + \sum_{j = 1}^{\infty} \frac{4 K}{(\pi j)^{2 + \epsilon}}
