...
Das parametrische diskrete Raum"=Zeit"=Variationsproblem lässt sich schreiben als
\begin{equation}
    \text{Sei } \bm \sigma \in \mathcal P, \text{ finde } u^{\mathcal{N}}(\bm \sigma) \in \mathcal{X} \colon \quad b(u^{\mathcal{N}}(\bm \sigma), v; \bm \sigma) = f(v) \quad \fa v \in \mathcal Y.
\end{equation}
Um diese nun in eine Form zu bringen, welche verwendet werden kann, um die Lösung $u^{\mathcal N}(\bm \sigma)$ zu bestimmen, setzen wir zunächst die Basen $\mathcal B^{\mathcal X}_{\mathcal N}$ und $\mathcal B^{\mathcal Y}_{\mathcal N}$ ein.
Unter Verwendung der Tensorprodukt-Darstellung der verwendeten Räume können wir ein Element $u^{\mathcal N} \in \mathcal X^{\mathcal N}$ schreiben als
\begin{equation}
    u^{\mathcal N} = \sum_{k = 0}^{\mathcal K} \sum_{j = 1}^{\mathcal J} u_{j}^{k} (\vartheta_{k} \otimes \eta_{j})
\end{equation}
und ein Element $v^{\mathcal N} = (y^{\mathcal I}, z^{\mathcal J}) \in \mathcal Y^{\mathcal N}$ als
\begin{equation}
...
\end{equation}
Dabei seien durch $\vec{u}^{\mathcal N} := (u^{k}_{j})_{j = 1, \dots, \mathcal J, k = 0, \dots, \mathcal K}$ sowie $\vec{y}^{\mathcal I} := (y^{k}_{j})_{j = 1, \dots, \mathcal J, k = 1, \dots, \mathcal K}$, $\vec{z}^{\mathcal J} := (z_{m})_{m = 1, \dots, \mathcal J}$ die Koeffizientenvektoren der jeweiligen Funktionen gegeben.

\paragraph{Systemmatrix} % (fold)
\label{par:systemmatrix}

% paragraph systemmatrix (end)

Durch anwenden der Bilinearformen $b_{i}$ auf diese Funktionen erhalten wir
\begin{align}
    ...
    \\&\quad= \begin{multlined}[t][.8\displaywidth]
    \sum_{\subalign{k &= 0\cr m &= 1}}^{\mathcal K} \sum_{j, n = 1}^{\mathcal J} u^{k}_{j} y^{m}_{n}
    ...
    + \sum_{k = 0}^{\mathcal K} \sum_{j,n = 1}^{\mathcal J} u^{k}_{j} z_{n} \skp{\theta_{k}(0) \eta_{j}}{\eta_{n}}{H}
    \end{multlined}
    \\&\quad=\begin{multlined}[t][.8\displaywidth]
    \sum_{\subalign{k &= 0\cr m &= 1}}^{\mathcal K} \sum_{j,n = 1}^{\mathcal J} u^{k}_{j} y^{m}_{n} \left[ \skp{\theta_{k}'}{\xi_{m}}{L_{2}(I)} \skp{\eta_{j}}{\eta_{n}}{H} + c \skp{\theta_{k}}{\xi_{m}}{L_{2}(I)}\skp{\grad \eta_{j}}{\grad \eta_{n}}{H} \right.
    \\\left. + \mu \skp{\theta_{k}}{\xi_{m}}{L_{2}(I)} \skp{\eta_{j}}{\eta_{n}}{H} \right] + \sum_{j,n = 1}^{\mathcal J} u^{0}_{j}  z_{n} \skp{\eta_{j}}{\eta_{n}}{H}
    \end{multlined}
\end{align}
und analog für $i = 1, \dots, N_{\mathcal P}$
\begin{align}
    b_{i}(u^{\mathcal N}, v^{\mathcal N})
    &= \int_{I} \chi_{i} \skp{\varphi_{i} u^{\mathcal N}}{y^{\mathcal I}}{H} \diff t
    \\&= \sum_{\subalign{k &= 0 \cr m &= 1}}^{\mathcal K} \sum_{j, n = 1}^{\mathcal J} u^{k}_{j} y^{m}_{n} \int_{I} \chi_{i} \skp{\varphi_{i} \theta_{k} \eta_{j}}{\xi_{m} \eta_{n}}{H} \diff t
    \\&= \sum_{\subalign{k &= 0 \cr m &= 1}}^{\mathcal K} \sum_{j, n = 1}^{\mathcal J} u^{k}_{j} y^{m}_{n} \skp{\chi_{i} \theta_{k}}{\xi_{m}}{L_{2}(I)} \skp{\varphi_{i} \eta_{j}}{\eta_{n}}{H}.
\end{align}

Definieren wir nun zunächst die benötigten Bausteine durch
\begin{equation}
    \mat{C}_{t}^{FE} := \left[ \skp{\theta_{k}'}{\xi_{m}}{L_{2}(I)} \right]_{\subalign{m &= 1, \dots, \mathcal K \cr k &= 0, \dots, \mathcal K}} \in \mathbb{R}^{\mathcal K \times \mathcal K + 1}, \qquad
    \mat{M}_{t}^{FE} := \left[ \skp{\theta_{k}}{\xi_{m}}{L_{2}(I)} \right]_{\subalign{m &= 1, \dots, \mathcal K \cr k &= 0, \dots, \mathcal K}} \in \mathbb{R}^{\mathcal K \times \mathcal K + 1},
\end{equation}
sowie die Anteile
\begin{equation}
    \mat{M}_{t}^{FE, i} := \left[ \skp{\chi_{i}\theta_{k}}{\xi_{m}}{L_{2}(I)} \right]_{\subalign{m &= 1, \dots, \mathcal K \cr k &= 0, \dots, \mathcal K}} \in \mathbb{R}^{\mathcal K \times \mathcal K + 1}, \quad i = 1, \dots, N_{\mathcal P},
\end{equation}
und
\begin{equation}
    \vec{e}_{t} := \left[ \delta_{0,k} \right]_{k = 0, \dots, \mathcal K}  \in \mathbb{R}^{1 \times \mathcal K + 1}
\end{equation}
und
\begin{equation}
    \mat{H}_{x} := \left[ \skp{\eta_{j}}{\eta_{n}}{H} \right]_{j,n=1, \dots, \mathcal J} \in \mathbb{R}^{\mathcal J \times \mathcal J},
    \qquad
    \mat{A}_{x} := \left[ \skp{\grad \eta_{j}}{\grad \eta_{n}}{H} \right]_{j,n=1, \dots, \mathcal J}  \in \mathbb{R}^{\mathcal J \times \mathcal J}
\end{equation}
und
\begin{equation}
    \mat{V}_{x} := \left[ \skp{\eta_{j}}{\eta_{n}}{V} \right]_{j,n = 1, \dots, \mathcal J} \in \mathbb{R}^{\mathcal J \times \mathcal J},
\end{equation}
wobei in unserem Setting stets $\mat{V}_{x} = \mat{H}_{x} + \mat{A}_{x}$ gilt,
und
\begin{equation}
    \mat{W}_{x}^{i} := \left[ \skp{\varphi_{i} \eta_{j}}{\eta_{n}}{H} \right]_{j, n = 1, \dots, \mathcal J}  \in \mathbb{R}^{\mathcal J \times \mathcal J}.
\end{equation}

Damit können wir die folgenden Raum"=Zeit"=Matrizen definieren:
\begin{equation}
...
\begin{equation}
    ...
    \mat{M}_{t}^{FE, i} \otimes \mat{W}_{x}^{i} \\
    \vec{0}
    \end{pmatrix}.
\end{equation}
und damit schlußendlich
\begin{equation}
...
% paragraph lastvektor (end)

Um die rechte Seite des Variationsproblems, das heißt,
\begin{equation}
    f(v) = \int_{I} \skp{g}{v_{1}}{V' \times V} \diff t + \skp{u_{0}}{v_{2}}{H}
\end{equation}
zu berechnen, gehen wir analog vor und verwenden für den Quellterm eine Trapez-Formel für das zeitliche Integral, dann gilt
\begin{align}
    f(v^{\mathcal N})
    ...
    \\&= \sum_{k = 1}^{\mathcal K} \sum_{j = 1}^{\mathcal J} \frac{\Delta t_{k}}{2} \skp{g(t_{k}) + g(t_{k - 1})}{\eta_{j}}{v' \times V}
...
Dies führt zum diskreten Lastvektor
\begin{equation}
    \vec{f} := \begin{pmatrix}
        \vec{g}^{1/2} \\
        \vdots\\
...
\paragraph{$\mathcal X$-Norm} % (fold)
\label{par:_mathcal_x_norm}

Weiter benötigen wir die Norm des Ansatzraums $\mathcal X$ für die gewählten Basisfunktionen.
Diese kann erneut durch Kronecker-Produkte von raum und zeit Matrizen dargestellt werden.
Betrachte dazu erneut die durch das Skalarprodukt \cref{eq:ansatzraum_skalarprodukt} induzierte Norm
\begin{equation}
    \norm{u}_{\mathcal X} = \left( \norm{u}_{L_{2}(I; V)}^{2} + \norm{u_{t}}_{L_2(I; V')}^2  \right)^{1/2}
\end{equation},
welche unter Verwendung von $u^{\mathcal N}$ geschrieben werden kann als
\begin{align}
    \norm{u^{\mathcal N}}_{\mathcal X}^{2}
    &= \int_{I} \norm{u^{\mathcal N}(t)}^2_{V} \diff t
        + \int_{I} \norm{u^{\mathcal N}_{t}(t)}_{V'}^2 \diff t
    \\&= \sum_{k,m = 0}^{\mathcal K} \sum_{j,n = 1}^{\mathcal J} u^{k}_{j} u^{m}_{n} \int_{I} \skp{\theta_{k} \eta_{j}}{\theta_{m} \eta_{n}}{V} \diff t
        + \sum_{k,m = 0}^{\mathcal K} \sum_{j,n = 1}^{\mathcal J} u^{k}_{j} u^{m}_{n} \int_{I} \skp{\theta_{k}' \eta_{j}}{\theta_{m}' \eta_{n}}{V'} \diff t
    \\&= \sum_{k,m = 0}^{\mathcal K} \sum_{j,n = 1}^{\mathcal J} u^{k}_{j} u^{m}_{n} \left[ \skp{\theta_{k}}{\theta_{m}}{L_{2}(I)} \skp{\eta_{j}}{\eta_{n}}{V} + \skp{\theta_{k}'}{\theta_{m}'}{L_{2}(I)} \skp{\eta_{j}}{\eta_{n}}{V'} \right]
\end{align}

Um das Ganze in eine handliche Matrix-Schreibweise zu verwandeln, müssen wir zunächst klären, wie die das diskrete $V'$-Skalarprodukt bestimmt werden kann.
Dazu verwenden wir \cref{lemma:berechnung_rieszsche_darstellung} um die Rieszsche Darstellung zu bestimmen, dann existiert ein $x_{j} \in V$ mit $\norm{x_{j}}_{V} = \norm{\eta_{j}}_{V'}$ und $\skp{x_{j}}{v}{V} = \skp{\eta_{j}}{v}{V' \times V}$ für alle $v \in V$.
Insbesondere gilt für die entsprechenden Koeffizientenvektoren $\mat{M} := (\vec{x}_{j})_{j = 1, \dots, \mathcal J}$ und $\mat H_{x}$ die Gleichung
\begin{equation}
    \mat{V}_{x} \vec x_{j} = \bm \eta_{j},
\end{equation}
also $\mat M = \mat{V}_{x}^{-1} \mat H_{x}$ und damit
\begin{equation}
    \skp{\eta_{h}}{\eta_{n}}{V'}
    = \skp{\mat{V}_{x}^{-1} \bm{\eta}_{j}}{\mat{V}_{x}^{-1} \bm{\eta}_{n}}{V}
    = (\mat{V}_{x}^{-1} \bm{\eta}_{j})\Transp \mat{V}_{x} (\mat(V)_{x}^{-1} \bm{\eta}_{n})
    = \bm{\eta}_{j}\Transp \mat{V}_{x}^{-1} \bm{\eta}_{n},
\end{equation}
oder zusammengefasst für alle $n$ und $j$ als diskrete Norm
\begin{equation}
    \mat{V}_{\mathrm{dual}} := \mat H_{x}\Transp \mat V_{x}^{-1} \mat H_{x}.
\end{equation}

Alles zusammen wird zu
\begin{equation}
    \mat X = \mat M_{t} \otimes \mat V_{x} + \mat A_{t} \otimes (\mat H_{x}\Transp \mat V_{x}^{-1} \mat H_{x}).
\end{equation}

% paragraph _mathcal_x_norm (end)

\paragraph{$\mathcal Y$-Norm} % (fold)
\label{par:_mathcal_y_norm}

Die $\mathcal Y$-Norm hat die Form
\begin{equation}
    \norm{v}_{\mathcal Y} = \left( \norm{v_{1}}^{2}_{L_{2}(I; V)} + \norm{v_{2}}^{2}_{H} \right)^{1/2},
\end{equation}
oder eingesetzt
\begin{align}
