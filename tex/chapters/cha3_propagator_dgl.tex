...
Damit können wir nun die folgende Operatorfamilie und die zugehörigen Bilinearformen definieren.
...
\label{bemerkung:raum_zeit_feld_norm_zeitunabhaengig}
    ...
    \end{equation}
    und somit wegen der Zerlegung $I = I_{1} \cupdot I_{2}$ insbesondere
    \begin{equation}
...
Motiviert durch das Iterationsverfahren aus \cref{chapter:einleitung}, in dem die Propagator"=Differentialgleichung immer wieder für leicht variierte Raum-Zeit-Felder $\omega$ berechnet wird, werden wir diese Felder als Ausgangspunkt der Parametrisierung verwenden.
...
Dies ist aber auch nicht nötig: Wie man in der Einführung bereits gesehen hat, weisen die während des Iterationsverfahrens tatsächlich auftretenden Felder gewisse Eigenschaften, beispielsweise Symmetrien, auf und wir können diese in die Wahl des Systems $\Set{\varphi_{j}}_{j \in \mathbb{N}}$ einfließen lassen.
...
Dies bedarf, wie zuvor, eines zeitlichen Wechsels zwischen mehreren Feldern $w_{i}$.
