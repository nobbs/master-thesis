...
wie in \cref{definition:gelfand_tripel} seien $V$ und $H$ zwei separable Hilberträume und $(V, H, V')$ das daraus resultierende Gelfand-Tripel.
...
\leavevmode
\begin{thmenumerate}
    \item Die Anfangswertbedingung $u(0) = u_{0}$ in $H$ ist wegen \cref{korollar:spursatz} wohldefiniert.
...
Hier kommen nun die in \cref{section:bochner_raeume} definierten Bochner-Räume zum Einsatz.

\begin{Definition}
\label{def:gl:le:ansatz_und_testraum}
    Als Ansatzfunktionenraum $\mathcal X$ bezeichnen wir den Raum $W(0, T; V, V')$ aus \cref{definition:bochner_raum_W}.
    Es ist also
    \begin{equation}
