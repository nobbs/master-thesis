...
\todo[inline]{Nochmal korrekturlesen!}
\todo[inline]{Eventuell auf verschiedene Kapitel aufteilen!}
\todo[inline]{Bei Bedarf erweitern!}
...
\todo[inline]{Nochmal nachschauen!}
...
\begin{Definition}
    ...
    \end{equation}
    gilt.
    Wir nennen \eqref{eq:grundlagen:hadamard_varprob} \emph{sachgemäß gestellt}, wenn eine eindeutige Lösung $u \in U$ existiert und diese eine A-priori-Abschätzung der Form
    \begin{equation}
...
Um dies für die Raum-Zeit-Variationsformulierung aus \thref{definition:variationsformulierung} nachzuweisen, werden wir den folgenden bekannten und wichtigen Satz, welcher in dieser oder ähnlicher Form bei \textcites[Theorem 2.1]{Babuska:1971fx}[Theorem 5.2.1]{Aziz:2014wf}[Theorem \S{}3.3.6]{Braess:2007wm} zu finden ist, verwenden.
...
\label{lemma:gl:lax_milgram}
    ...
    \begin{Beweis}
        ...
        \end{equation}
        Damit sind neben der Stetigkeit von $a$ auch die die Bedingungen \ref{satz:gl:bnb_theorem:bedingung:inf_sup} und \ref{satz:gl:bnb_theorem:bedingung:surjektiv} aus \thref{satz:gl:bnb_theorem} erfüllt und die Aussage folgt aus dem Banach-Ne{\v c}as-Babu{\v s}ka-Theorem.
    \end{Beweis}
\end{Lemma}
...
\todo[inline]{Ungleichung der stetigen Abhängigkeit einbauen}
\todo[inline]{Nochmal nachrechnen}
\begin{Korollar}
...
\todo[inline]{Beweise ausführlich aufschreiben?}
...
% \begin{Definition}
% \label{definition:gl:ansatz_und_testraum_homogen}
%     Der Ansatzfunktionenraum $\hat{\mathcal X}$ sei als der folgende Unterraum von $W(0, T; V, V')$
%     \begin{equation}
%         \label{eq:var_all_ansatzraum_x_homogen}
%         \begin{aligned}
...
%     \begin{equation}
%         \label{eq:var_all_ansatzraum_x_norm_homogen}
...
%     \begin{equation}
%         \label{eq:var_all_testraum_y_homogen}
%         \hat{\mathcal Y} = L_{2}(0, T; V)
%     \end{equation}
%     mit der üblichen Norm
%     \begin{equation}
%         \label{eq:var_all_testraum_y_norm_homogen}
%         \norm{v}_{\hat{\mathcal Y}} = \norm{v_{1}}_{L_{2}(0, T; V)}, \quad v \in \hat{\mathcal Y}.
%     \end{equation}
% \end{Definition}

% \begin{Definition}
% \label{definition:variationsformulierung_homogen}
%     Seien $\hat{\mathcal X}$ und $\hat{\mathcal Y}$ wie in~\eqref{eq:var_all_ansatzraum_x_homogen} respektive~\eqref{eq:var_all_testraum_y_homogen}.
%     Als \emph{homogenisierte Raum-Zeit-Variations"-for"-mu"-lie"-rung} der linearen Evolutionsgleichung~\eqref{eq:allgemeine_parabolische_pde} bezeichnen wir das folgende Problem:
...
%     Dann gilt: $\hat u \in \tilde{\mathcal X}$ ist genau dann eine Lösung des homogenisierten RZVP aus \thref{definition:variationsformulierung_homogen}, wenn $u := \hat u + \hat u_{0} \in \mathcal X$ eine Lösung  des RZVP aus \thref{definition:variationsformulierung} ist.
...
\todo[inline]{Abschnitt sauber aufziehen, d.h. Formulierungen verbessern, eventuell ausführlicher.}
...
Unter gewissen Voraussetzungen, zum Beispiel, wenn die Bilinearform $a$ die Bedingungen des Banach-Ne{\v c}as-Babu{\v s}ka-Theorems (\thref{satz:gl:bnb_theorem}) erfüllt, existiert eine eindeutige Lösung $u \in U$ des obigen Variationsproblems.
...
Zu Beachten ist, dass selbst wenn das eigentliche Variationsproblem \eqref{eq:gv:variationsproblem} eindeutig lösbar ist, dies noch nicht für das diskretisierte Variationsproblem \eqref{eq:gv:variationsproblem_diskret} gelten muss.
...
\begin{Satz}
    ...
    Weiter seien $a \colon U \times V \to \mathbb{R}$ eine Bilinearform und $f \colon V \to \mathbb{R}$ ein lineares Funktional, wobei $a$ die Bedingungen aus \thref{satz:gl:bnb_theorem} erfülle.
    ...
    Erfüllt $a_{n, m}$ die Bedingungen \ref{satz:gl:bnb_theorem:bedingung:inf_sup} und \ref{satz:gl:bnb_theorem:bedingung:surjektiv} aus \thref{satz:gl:bnb_theorem} mit der inf-sup-Konstante $\alpha_{n, m} > 0$ statt $\alpha$, dann existiert eine eindeutige Lösung $u_{n} \in U_{n}$ des Variationsproblems \eqref{eq:gv:variationsproblem_diskret} und diese erfüllt
    \begin{equation}
