...
Für letztere weisen wir abschließend nach, dass sie sachgemäß gestellt ist und unter zusätzlichen Bedingungen eine gewisse Regularität bezüglich der Parameter aufweist.
...
% TODO: Definition anhand bemerkung anpassen
...
Der nächste Schritt ist nun, nachzuweisen, dass die obige Raum"=Zeit"=Variationsformulierung sachgemäß gestellt ist im Sinne von Hadamard, \cref{definition:sachgemaess_gestellt_nach_hadamard}, also eine eindeutige Lösung besitzt, welche stetig von dem Funktional $f \in \mathcal Y'$ abhängt.
...
Dies erreichen wir durch folgende Einschränkung der verwendeten Felder $w \in L_{\infty}(\Omega)$ auf Funktionen, die wir als Reihenentwicklung von hier noch nicht näher spezifizierten Funktionen $\varphi_{j}$ darstellen können.

\begin{Definition}
\label{definition:feld_entwickelbar}
    Sei $\Set{\varphi_{j}}_{j \in \mathbb{N}} \subset L_{\infty}(\Omega)$ ein System von Funktionen und sei weiter ein Parameterraum $\mathcal P \subset \ell_{\infty}(\mathbb{N})$ gegeben.
    ...
    \begin{equation}
        w(\bm\sigma) = \sum_{j = 1}^{\infty} \sigma_{j} \varphi_{j}
    \end{equation}
...
Genauer gehen wir hierauf in der Einführung in diese Methode in \cref{??} ein.
...
Bisher mussten keine zusätzlichen Bedingungen gefordert werden, um die Aussagen zu beweisen.
Dies ändert sich an dieser Stelle.
Zunächst aber wollen wir darauf eingehen, wie die obigen Aussagen dazu führen, die analytische Abhängigkeit der Lösung vom Parameter nachzuweisen.
