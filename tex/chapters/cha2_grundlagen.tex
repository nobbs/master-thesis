...
\label{thm:schwab09:theorem51:ungleichungen}
    Unter den Gegebenheiten aus \cref{thm:schwab09:theorem51} gilt ferner
    \begin{equation}
    ...
    \end{equation}
    definiert sind.
    % Nach \cref{korollar:gl:einbettungskonst_M_e} ist $M_{e}$ gleichmäßig beschränkt in der Wahl von $V \hookrightarrow H$ und nur im Fall $T \to 0$ von $T$ abhängig.
\end{Korollar}
...
%     Seien $\hat{\mathcal X}$ und $\hat{\mathcal Y}$ wie in~\cref{eq:var_all_ansatzraum_x_homogen} respektive~\cref{eq:var_all_testraum_y_homogen}.
%     Als \emph{homogenisierte Raum-Zeit-Variations"-for"-mu"-lie"-rung} der linearen Evolutionsgleichung~\cref{eq:allgemeine_parabolische_pde} bezeichnen wir das folgende Problem:
...
%     Dann gilt: $\hat u \in \tilde{\mathcal X}$ ist genau dann eine Lösung des homogenisierten RZVP aus \cref{definition:variationsformulierung_homogen}, wenn $u := \hat u + \hat u_{0} \in \mathcal X$ eine Lösung  des RZVP aus \cref{definition:variationsformulierung} ist.
...
Zu Beachten ist, dass selbst wenn das eigentliche Variationsproblem \cref{eq:gv:variationsproblem} eindeutig lösbar ist, dies noch nicht für das diskretisierte Variationsproblem \cref{eq:gv:variationsproblem_diskret} gelten muss.
