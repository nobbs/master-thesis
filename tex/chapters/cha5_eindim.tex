...
\begin{Satz}
    ...
    Weiter sei $\Set{\hat A, A_{j} \given j \in \mathbb{N}_{0}}$ die affine Zerlegung von $A = -c \Delta + \omega$ wie in \cref{eq:1d:affine_zerlegung} und es gelte \thref{satz:regularitaet_nachrechnen}.
...
\begin{enumerate}
    \item Wohldefiniertheit der PDE \cref{eq:parabolische_pde}, das heißt die Voraussetzungen von \thref{satz:gl:le:ss09_theorem51} nachweisen. Weiterhin lassen sich damit die inf-sup-Bedingung von \cref{eq:ps:rzvp:schwache_formulierung} nachrechnen und damit die Schranken für $B$ und $B^{-1}$ bestimmen.
