...
Wir greifen nun die in \cref{chapter:einleitung} eingeführten parabolischen Differentialgleichungen \cref{eq:el:forward_propagator,eq:el:backward_propagator} auf, welche von den beiden Propagatoren $q$ und $q^{\dagger}$ erfüllt werden.
...
\label{bemerkung:raeume_und_gelfand_tripel}
    ...
    \begin{equation}
        V \denseinclusion H \simeq H' \denseinclusion V' = (H^{1}_{0}(\Omega))' = H^{-1}(\Omega)
    \end{equation}
    ein Gelfand"=Tripel, vergleiche \cref{def:gl:br:gelfand_tripel}.
    Motiviert durch \cref{bemekerung:duality_pairing_stetig_und_so} verwenden wir die Schreibweise $\skp{\blank}{\blank}{V' \times V}$ auch für die duale Paarung auf $V' \times V$.
\end{Bemerkung}
...
\label{eq:bilinearform_formel}
    ...
    \end{aligned}
\end{equation}
wobei bei der Wechsel von dualer Paarung auf $V' \times V$ zum $H$-Skalarprodukt beim zweiten und dritten Summanden durch \cref{bemekerung:duality_pairing_stetig_und_so} und die Tatsache, dass mit $\eta \in V \subset H$ und der \cref{definition:feld_raum_zeit_feld} von $\omega$ auch $\omega(t, \blank) \eta \in H$ gilt, gerechtfertigt ist.
...
\label{satz:bilinearform_messbar_stetig_garding}
    ...
    \begin{thmenumerate}
        ...
        \emph{Messbarkeit}: die Abbildung $I \ni t \mapsto a(\blank, \blank; t)$ ist messbar.
    ...
    \begin{Beweis}
        Die Messbarkeit folgt aus ... irgendwas!
        ...
        \begin{align}
            a(\eta, \eta; t) + \lambda \norm{\eta}^{2}_{H}
            &\geq c \norm{\grad \eta}^{2}_{H},
            \intertext{woraus wir durch Anwenden der Poincaré"=Friedrichs"=Ungleichung, \cref{satz:poincare_ungleichung},}
            a(\eta, \eta;t ) + \lambda \norm{\eta}^{2}_{H}
...
Es bietet sich an, die aus der \cref{def:gl:le:ansatz_und_testraum} bereits bekannten Räume
\begin{equation}
...
Der nächste Schritt ist nun, nachzuweisen, dass die obige Raum"=Zeit"=Variationsformulierung sachgemäß gestellt ist im Sinne von Hadamard, \cref{def:gl:le:hadamard_sachgemaess_gestellt}, also eine eindeutige Lösung besitzt, welche stetig von dem Funktional $f \in \mathcal Y'$ abhängt.
Hierzu werden wir mit \cref{satz:gl:le:ss09_theorem51} ansetzen, welcher unter den gegebenen Rahmenbedingungen die zu prüfenden Bedingungen auf die in \cref{satz:bilinearform_messbar_stetig_garding} bereits nachgewiesenen reduziert.
...
\label{satz:raum_zeit_variationsformulierung_sachgemaess_gestellt}
    ...
    \begin{Beweis}
        Folgt nach Definition der Raum"=Zeit"=Variationsformulierung unmittelbar aus \cref{satz:gl:le:ss09_theorem51} und \cref{satz:bilinearform_messbar_stetig_garding}.
    \end{Beweis}
\end{Korollar}
...
\label{korollar:rz_variationsformulierung_stetig_infsup_schranken}
    ...
    \begin{align}
        ...
        \beta^{-1} &\leq \frac{\sqrt{2 \max\Set{c^{-2}\gamma_{\Omega}^{-4}, 1} + M_{e}^{2}}}{\gamma_{\Omega}^{2} \min\Set{c, c^{-1}, c (\norm{\omega}_{L_{\infty}(I; L_{\infty}(\Omega))} + \abs{\mu})^{-2}}}.
    \end{align}
...
Wie zuvor wollen wir \cref{satz:gl:le:ss09_theorem51} verwenden, müssen also die Bedingungen an $a(\blank, \blank;t, \bm\sigma)$ nachweisen.
...
\label{satz:parametrische_bilinearform_messbar_stetig_garding}
    ...
    \begin{thmenumerate}
        ...
        \emph{Messbarkeit}: die Abbildung $I \ni t \mapsto a(\blank, \blank; t, \bm\sigma)$ ist messbar.
...
\label{satz:existenz_partieller_ableitungen}
    ...
    \begin{Beweis}
        ...
        \end{equation}
        die schwache Formulierung nach wie vor sachgemäß gestellt ist.
...
Wählen wir also $V = H^{1}_{\text{per}}(\Omega)$ und $H = L_{2,\text{per}}(\Omega)$, dann erhalten wir nach \cref{def:gl:br:gelfand_tripel} wie zuvor ein Gelfand-Tripel der Form
\begin{equation}
    V \denseinclusion H \simeq H' \denseinclusion V' = (H^{1}_{\text{per}}(\Omega))'.
\end{equation}
